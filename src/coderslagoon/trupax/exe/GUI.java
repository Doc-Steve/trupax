...
public class GUI extends Exe implements NLS.Reg.Listener {
    ...
    void initialize(String[] args) throws ExitError {
        ...
        // HACK: enable VeraCrypt by default
        if (null == this.prg.getProperty(Prg.Prop.FOR_VERACRYPT)) {
            this.prg.setProperty(new Prg.NamedString(Prg.Prop.FOR_VERACRYPT,
                                                     Boolean.TRUE.toString()));
        }
    ...
    void registerObjects(String[] objs) {
        ...
        Prg.RegisterObjectsCallbacks rocb = new
        Prg.RegisterObjectsCallbacks() {
    ...
    void make() {
        ...
        Prg.MakeCallback mcb = new Prg.MakeCallback() {
            ...
            @Override
            public Prg.Result onFreeSpace() {
                this.pc.onFile(null, -1L);
                return Prg.Result.ok();
            }
            @Override
            public Prg.Result onKeyGen() {
                this.pc.onFile(NLS.GUI_PROGRESS_KEYGEN.s(), -1L);
                return Prg.Result.ok();
            }
        };
        pdlg.v.open();
